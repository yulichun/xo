...
export const parseOptions = (
  ...
  const textColor =
    colorMode == ColorMode.Light
      ? customColors.textColorRGB.light
      : customColors.textColorRGB.dark
  const axesColor =
    colorMode == ColorMode.Light
      ? 'rgba(0, 10, 23, 0.09)'
      : 'rgba(240, 250, 255, 0.09)'
  ...
  const negativeYTarget = config.panel.overrides.find((override) =>
    findRuleInOverride(override, GraphRules.SeriesNegativeY),
  )
  const negativeYUnit = findRuleInOverride(
    negativeYTarget,
  ...
  const yAxis = [
    {
      ...
      values: (u, vals) =>
        vals.map((v) => {
          ...
          return (
            formatUnit(v, units, config.panel.plugins.graph.value.decimal) ??
            round(v, config.panel.plugins.graph.value.decimal)
          )
        }),
  ...
  rawData.forEach((d, i) => {
    ...
    if (yOverride) {
      ...
      yAxis.push({
        ...
        values: (u, vals) =>
          vals.map((v) => {
            return (
              formatUnit(v, unitsOverride?.units, decimal) ?? round(v, decimal)
            )
          }),
        units: unitsOverride,
        side: 1,
    ...
    if (
      style == 'points' ||
      config.panel.plugins.graph.styles.showPoints == 'always'
    ) {
      pointCfg = {
        show: true,
    ...
    if (!isEmpty(inactiveSeries)) {
      ...
      showSeries = !defaultLegend || d.name.match(defaultLegend)
    }
    series.push({
      show:  showSeries,
      label: d.name,
      ...
      width: lineWidth,
      fill:
        config.panel.plugins.graph.styles?.gradientMode == 'none'
          ? colorManipulator.alpha(d.color ?? '', opacity)
          : fill(d.color, opacity),
      spanGaps: config.panel.plugins.graph.styles.connectNulls,
      paths:
        style == 'bars'
          ? uPlot.paths.bars({
              size: [
                1 - config.panel.plugins.graph.styles.barGap / 100,
                BardMaxWidth,
              ],
              align: -1,
